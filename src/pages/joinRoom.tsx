...
export default function JoinRoomScreen() {
  ...
  const handleJoinGame = () => {
    if (playerName.trim()) {
      // ここでゲーム参加処理を実装
      // 例: router.push(`/waitingForOpponent?roomId=${FIXED_ROOM_ID}&playerName=${playerName}`)
    }
  };
    ...
    <Layout>
      ...
      <div className="bg-white rounded-2xl shadow-lg w-80 flex flex-col items-center px-6 py-6 mb-4">
        ...
        <input
          ...
          placeholder="User 2"
          className="w-full h-10 bg-gray-50 rounded-xl border-2 border-gray-200 px-4 text-base font-semibold focus:outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-200 transition placeholder-gray-400 mb-4"
        />
        {/* ゲーム参加ボタン */}
        <button
